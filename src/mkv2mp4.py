...
def mkv_to_mp4(input_file: str, output_file: str = None) -> bool:
    ...
    if not input_path.suffix.lower() == '.mkv':
        console.print(f"[yellow]Warning:[/yellow]\t\tInput file is not .mkv: {input_file}")
    ...
    console.print(f"[cyan]\t\tConverting:[/cyan] {input_path.name} → {output_path.name}")
    ...
    if result.returncode == 0:
        ...
        try:
            ...
            console.print(f"[yellow]\t\tWarning:[/yellow] \t\tCouldn't remove input file: {e}")
...
def main():
    ...
    parser.add_argument(
        '-o', '--output',
        help="Output MP4 file (only for single file conversion)"
    )

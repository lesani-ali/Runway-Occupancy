...
def process_video(
    ...
    console.print(
        f"[cyan][INFO][/cyan]\t\t[green]Loading model {config.weights} "
        f"on {config.device} (SAHI: {'yes' if use_sahi else 'no'})[/green]"
    )
    detection_model = load_detection_model(
        weights=config.weights, device=config.device,
        conf=config.conf, use_sahi=use_sahi,
    )
    console.print(
        f"[cyan][INFO][/cyan]\t\t[green]Device: {detection_model['device']}[/green]"
    )
    ...
    with Progress(
        ...
        while cap.isOpened():
            ...
            if frame_index % stride == 0:
                ...
                frame_for_det = frame
                if roi is not None and config.mask_roi:
                ...
                if config.save_img and save_dir is not None:
                    _save_annotated_frame(frame, raw_results, processed, save_dir, config.hide_conf)
...
def _save_annotated_frame(
    ...
    for box, score in zip(raw_results["boxes"], raw_results["scores"]):
        ...
        if not hide_conf:
            cv2.putText(vis, f"{score:.2f}", (x1, y1 - 10),
